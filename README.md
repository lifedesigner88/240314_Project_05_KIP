...
기존 핵심 기능은 유지하고, 인프라 복잡도는 줄이는 방향 권장.

### 목표
- 면접용 데모에 필요한 핵심 기능만 안정적으로 유지.
- AWS 관리형 서비스 의존성 축소 및 운영비 최소화.
- 누구나 링크로 접속해 주요 시나리오를 확인할 수 있는 구조로 단순화.

### 권장 방향
- 프론트엔드는 정적 호스팅으로 분리.
- 백엔드는 단일 서버 또는 단일 VM 기반으로 단순화.
- DB는 컨테이너 1개로 함께 운영 가능한 구조 유지.
- `OpenSearch`, `Redis`, `FCM`, `S3`는 데모 범위에 맞게 비활성화 또는 대체.

### 구체적인 정리 기준
- 검색: `OpenSearch` 대신 DB 기반 검색 또는 제목/본문 단순 검색으로 축소.
- 첨부파일: `S3` 대신 로컬 볼륨 또는 단일 스토리지로 대체.
- 푸시 알림: `Firebase + Redis` 대신 비활성화 또는 인앱 알림만 유지.
- 배포: `Kubernetes` 대신 `Docker Compose` 수준으로 축소.

### 최종적으로 남기면 좋은 데모 시나리오
1. 관리자 로그인
2. 사용자 생성
3. 그룹 생성 및 사용자 배치
4. 문서 작성 및 수정
